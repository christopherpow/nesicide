...
def create_optparser():
    ...
    p = optparse.OptionParser(usage="python %prog [options]",
            version="2.9.1-snapshot-3ff05a0ef88d")
...
def check_qscintilla():
    ...
    if os.access(sciglobal, os.F_OK):
        ...
        if glob.glob(os.path.join(opts.qscilibdir, "*qscintilla2*")):
            ...
            # versioning.
            if sciversstr != "2.9.1-snapshot-3ff05a0ef88d":
                sipconfig.error("QScintilla %s is being used but the Python bindings 2.9.1-snapshot-3ff05a0ef88d are being built.  Please use matching versions." % sciversstr)
