...
CNameTablePreviewRenderer::~CNameTablePreviewRenderer()
{
   glDeleteTextures(1,&textureID);
}

void CNameTablePreviewRenderer::initializeGL()
{
...
void CNameTablePreviewRenderer::setBGColor(QColor clr)
{
   glClearColor((float)clr.red() / 255.0f, (float)clr.green() / 255.0f, (float)clr.blue() / 255.0f, 0.5f);
}

void CNameTablePreviewRenderer::resizeGL(int width, int height)
{
   QSize actualSize;
   ...
   glScalef( actualSize.width() / float( width )/512, actualSize.height() / float( height )/480, 1 );
...
void CNameTablePreviewRenderer::paintGL()
{
   glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
   glBindTexture (GL_TEXTURE_2D, textureID);
   glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, 512, 512, GL_RGBA, GL_UNSIGNED_BYTE, imageData);
   glBegin(GL_QUADS);
