...
public:
   ...
   // inspector.
   static inline uint16_t _SCROLLX ( int32_t x, int32_t y ) { return nesGetScrollXAtXY(x,y); }
   static inline uint16_t _SCROLLY ( int32_t x, int32_t y ) { return nesGetScrollYAtXY(x,y); }

   // Accessor functions for the pixel coordinate of the last sprite-0 hit.
   static inline void _SPRITE0HIT ( uint8_t* x, uint8_t* y ) { nesGetLastSprite0Hit(x,y); }
